...
class VanillaRNN(object):
    ...
    def step_forward(self, x, prev_h):
        ...
        ############################################################################
        next_h = np.tanh(prev_h@self.params[self.wh_name]
                        + x@self.params[self.wx_name]
    ...
    def step_backward(self, dnext_h, meta):
        ...
        dact = 1 - np.square(next_h) # Deritive w/ respect to activation function

        dx = (dnext_h*dact)@(self.params[self.wx_name].T) 

        dprev_h = (dnext_h*dact)@(self.params[self.wh_name].T)
        
        dWx = x.T@(dnext_h*dact)

        dWh = prev_h.T@(dnext_h*dact)

        db = (dnext_h*dact).sum(axis=0)
    ...
    def backward(self, dh):
        ...
        total_gradient = dh[:,-1,:] # Final state doesn't have "inflow from next state"
        ...
        for i in range(T - 1, -1, -1):
            ...
            total_gradient = dprev_h # Gradient flowing from right to left 

            if i != 0: # "initial state" doesn't have an output
                total_gradient += dh[:,i-1,:]

        dh0 = dprev_h # Optimizing the initial state
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        a = (x@self.params[self.wx_name] + prev_h@self.params[self.wh_name] 
            + self.params[self.b_name])
        ...
        next_c = f*prev_c + i*g  # How much of old state should be kept versus
                                 # Derived from input at new state
                                 # And what information from input should be 
                                 # considered

        next_h = o*np.tanh(next_c) # 'Processing' for next time-step
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        x, prev_h, prev_c, i, f, o, g, tan_of_next_c = meta

        do = tan_of_next_c*dnext_h
        dnext_c_from_current_state = o*(1 - tan_of_next_c**2)*dnext_h

        dnext_c = dnext_c_from_current_state + dnext_c
    ...
    def backward(self, dh):
        ...
        for i in range(T - 1, -1, -1):
            dx_i, dprev_h, dprev_c, dWx, dWh, db = self.step_backward(total_gradient, 
                                                                        dprev_c, self.meta[i])
