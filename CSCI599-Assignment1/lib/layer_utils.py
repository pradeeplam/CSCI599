...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        self.grads[self.b_name] = np.ones(feat.shape[0])@dprev
...
class relu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dfeat = (feat > 0.0).astype(float)*dprev
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True):
        ...
        if is_training and self.keep_prob != 0.0:
            kept = np.random.binomial(1, self.keep_prob, feat.shape) * (1/self.keep_prob)
        else:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        loss = -np.sum(one_hot*np.log(logit),axis=1)
    ...
    def backward(self):
        ...
        #############################################################################
        one_hot = np.zeros(logit.shape)
        one_hot[np.arange(label.shape[0]), label] = 1

        dlogit = -(one_hot - logit)
        
        if self.size_average:
            dlogit = dlogit/dlogit.shape[0]
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
